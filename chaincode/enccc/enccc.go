...
// CORE_PEER_ADDRESS=peer:7052 CORE_CHAINCODE_ID_NAME=encc:0 ./enccc
// peer chaincode install -p chaincodedev/chaincode/enccc -n encc -v 0
// peer chaincode instantiate -n encc -v 0 -C myc -c '{"Args":[]}'
// ENCKEY=`openssl rand 32 -base64` && DECKEY=$ENCKEY && IV=`openssl rand 16 -base64`
// peer chaincode invoke -n encc -C myc -c '{"Args":["encRecord","1009","2006"]}' --transient "{\"ENCKEY\":\"$ENCKEY\",\"IV\":\"$IV\"}"
// peer chaincode query -n encc -C myc -c '{"Args":["decRecord","1009"]}' --transient "{\"DECKEY\":\"$DECKEY\"}"
